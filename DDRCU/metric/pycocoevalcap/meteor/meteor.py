...
class Meteor:
    def __init__(self):
        ...
        meteor_cmd = ['/usr/bin/java', '-jar', '-Xmx2G', METEOR_JAR, '-', '-', '-stdio', '-l', 'en', '-norm', '-a', 'data/paraphrase-en.gz']
        env = os.environ.copy()
        env['LC_ALL'] = "C"
        try:
            ...
            raise RuntimeError(f"Failed to start Java process: {e}")
    ...
    def _stat(self, hypothesis_str, reference_list):
        ...
        try:
            ...
            # 디버깅 정보를 출력
            print("Java process status:", self.meteor_p.poll())
            if self.meteor_p.poll() is not None:
                print("Java process already terminated!")
            stderr_output = self.meteor_p.stderr.read() if self.meteor_p.stderr else "No stderr output"
            raise RuntimeError(
                f"Error~~~~~~~~~~~~~~~~~~~: {e}. Java stderr: {stderr_output}"
            )
