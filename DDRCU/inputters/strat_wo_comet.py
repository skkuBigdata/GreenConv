...
class InputFeatures(object):
    def __init__(
        ...
        # self.comet_inputs_ids = comet_inputs_ids
        # self.comet_inputs_length = len(comet_inputs_ids)

        # self.padding_length = max(self.input_length, self.persona_input_length, self.comet_inputs_length)
...
def featurize(
    ...
    if concat_ctx_persona:
        ...
        if not encode_context:
            ...
            context = '</s> <s>'.join(context)
            context = toker(context)
            persona_input_ids = toker(persona).input_ids[-max_input_length:]
            ...
            # comet_inputs_ids = []
            # for com in comet:
            #     comet_inputs_ids.append(toker(com).input_ids)
...
def convert_data_to_inputs(data, toker: PreTrainedTokenizer, use_all_persona, encode_context, **kwargs):
    ...
    for i in range(len(dialog)):
        ...
        # if i > 0 and dialog[i]['speaker'] == 'sys' and dialog[i - 1]['speaker'] != 'sys':
            ...
            if not use_all_persona:
                if use_prompt:
                    ...
                    if user_number > 2:
                        ...
                        #comet = dialog[i]['comet']
                        # intent = comet[0]
                        # need = comet[1]
                        # want = comet[2]
                        # effect = comet[3]
                        # react = comet[4]
                        # relation = ['intent', 'need', 'want', 'effect', 'react']
                        # com = "<persona> "
                        # for index, rel in enumerate(comet[:4]):
                        #     c = []
                        #     for item in rel:
                        ...
                        #             item = item.translate(str.maketrans('', '', punctuation))
                        #             c.append(item)
                        #     com.append(relation[index] + ': ' + ', '.join(c))
                        ...
                        if len(dialog[i]) > 3:
                            ...
                            for dpr in dialog[i]['dpr'][:3]:
                                ...
                                # ICL += 'System: [' + dpr[1] + '] ' + dpr[2] + "<input>"csv
                            # ICL += persona
                        else:
                            ICL = "<input>"
                    else:
...
class FeatureDataset(Dataset):
    ...
    def collate(features: List[InputFeatures], toker: PreTrainedTokenizer, infer=False):
        ...
        # for i in range(4):
        #     comet_input_ids.append(my_pad_sequence([torch.tensor(f.comet_inputs_ids[i], dtype=torch.long) for f in features],
        #                                       batch_first=True, max_len=max_len, padding_value=pad))
        #     comet_attention_mask.append(my_pad_sequence(
        #         [torch.tensor([1.] * len(f.comet_inputs_ids[i]), dtype=torch.float) for f in features],
        #         batch_first=True, max_len=max_len, padding_value=0.))

        # intent_input_ids = my_pad_sequence([torch.tensor(f.comet[0], dtype=torch.long) for f in features],
        #                                    batch_first=True, max_len=max_len, padding_value=pad)
        # intent_attention_mask = my_pad_sequence([torch.tensor([1.] * len(f.comet[0]), dtype=torch.float) for f in features],
        #                                         batch_first=True, max_len=max_len, padding_value=0.)
        # need_input_ids = my_pad_sequence([torch.tensor(f.comet[1], dtype=torch.long) for f in features],
        #                                    batch_first=True, max_len=max_len, padding_value=pad)
        # need_attention_mask = my_pad_sequence([torch.tensor([1.] * len(f.comet[1]), dtype=torch.float) for f in features],
        #                                         batch_first=True, max_len=max_len, padding_value=0.)
        # want_input_ids = my_pad_sequence([torch.tensor(f.comet[2], dtype=torch.long) for f in features],
        #                                    batch_first=True, max_len=max_len, padding_value=pad)
        # want_attention_mask = my_pad_sequence([torch.tensor([1.] * len(f.comet[2]), dtype=torch.float) for f in features],
        #                                         batch_first=True, max_len=max_len, padding_value=0.)
        # effect_input_ids = my_pad_sequence([torch.tensor(f.comet[3], dtype=torch.long) for f in features],
        #                                    batch_first=True, max_len=max_len, padding_value=pad)
        # effect_attention_mask = my_pad_sequence([torch.tensor([1.] * len(f.comet[3]), dtype=torch.float) for f in features],
        #                                         batch_first=True, max_len=max_len, padding_value=0.)
