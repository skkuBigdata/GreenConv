...
def convert_data_to_inputs(data, toker, prepare_persona_ahead, model, tokenizer, **kwargs):
    """
    데이터를 입력 형태로 변환하는 함수. 각 대화를 개별적으로 처리하도록 수정.
    """
    process = lambda x: toker.convert_tokens_to_ids(toker.tokenize(x))

    dialog = data['dialog']
    user_dialog = []
    persona = None  # 초기 persona 값
    inputs = []  # 최종 입력 리스트
    context = []  # 히스토리 저장 리스트
    ...
    for i in range(len(dialog)):
        text = _norm(dialog[i]['text'])  # 대화 텍스트 정규화

        # seeker의 대화 처리
        if dialog[i]['speaker'] == 'seeker':
            user_dialog.append(text)  # 페르소나 추출용 데이터 업데이트
            if add_speaker:
                text = "Persona:" + text

            # 대화를 개별적으로 모델에 전달해 persona 생성
            if not prepare_persona_ahead:
                try:
                    persona_input = text + tokenizer.eos_token
                    ...
                    persona_input_ids = persona_input_ids.to('cuda')

                    # 모델을 사용해 Persona 생성
                    persona_output = model.generate_text(persona_input_ids, 10)
                    print(f"persona_output: {persona_output}")  # 디버깅용 출력

                    if persona_output:
                        new_infer_res = filter_persona(persona_output[0])
                        print(f"filtered_persona: {new_infer_res}")  # 디버깅용 출력
                        persona_segments = new_infer_res.split("<persona>")
                        persona_segments = [seg.strip() for seg in persona_segments if seg.strip()]  # 빈 값 제거

                        if persona_segments:
                            persona = "<persona>" + "<persona>".join(persona_segments) + "<input>"
                        else:
                            persona = "<input>"
                    else:
                        persona = "<input>"
    ...
    res = {
        'context': history_dialog,  # 대화 히스토리
        'persona': persona          # 최종 persona 값
    }
    inputs.append(res)
