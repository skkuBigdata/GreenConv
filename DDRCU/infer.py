...
for infer_idx, infer_input_file in enumerate(args.infer_input_file): 
    ...
    metric_res = {}
    if not args.only_encode and not args.only_generate: # evaluate the model 
    ...
    for batch, posts, references, sample_ids in infer_dataloader:
        ...
        if 'other_res' in batch:
            ...
            if add_acc:
                ...
                for k, v in batch_other_res['acc_map'].items():
                    ...
                    if f'{v}_top1' in encoded_info: #만약 top1이 있다면 
                        encoded_info[f'{v}_top1'] = encoded_info[f'{v}_top1'].tolist() #list로 변환
                    ...
                    if k not in other_res['acc']: #만약 k가 acc에 없다면 
                        other_res['acc'][k] = [] #k를 acc에 추가
                    other_res['acc'][k].extend(batch_other_res[k]) #batch_other_res[k]를 acc[k]에 추가
        ...
        for idx in range(len(sample_ids)):
            ...
            if not args.only_encode:
                ...
                # tmp_persona = toker.decode(persona[0][idx], skip_special_tokens=True)
                # print(tmp_persona)
    ...
    for item in res:
        ...
        dialog[sample_ids].append("System: " + item['generation'])
    ...
    if args.add_nlg_eval:
        ...
        for line in res:
            ...
            ref_list.append([ref])  # sacrebleu는 다중 참조를 리스트로 받음
            hyp_list.append(hyp)
        ...
        metric_res = {
            ...
            "Hypothesis Length": bleu.sys_len,
            "Reference Length": bleu.ref_len,
        }
        ...
        metric_res_list = [
            ...
            for hyp, refs in zip(hyp_list, ref_list)
        ]
        ...
        with open(os.path.join(save_dir, f"metric_nlgeval_list.json"), "w") as f:
            json.dump(metric_res_list, f, ensure_ascii=False, indent=2)
