...
for name, param in model.named_parameters():
    if param.grad is not None:
        if torch.isnan(param.grad).any():
            logger.error(f"NaN in gradient for {name}.")
        if torch.isinf(param.grad).any():
            logger.error(f"Inf in gradient for {name}.")
...
output_dir = join(f'./DATA/{args.inputter_name}.{args.config_name}_model',    #_new_split_usr_sys',
                    ...
                    if args.num_epochs is None and global_step % args.valid_step == 0:  # and epoch > 0:
                        ...
                        if args.local_rank == -1 or get_rank() == 0:
                            try:
                                ...
                                logger.error(f"Error during evaluation or saving: {e}")
                            finally:
                                model.train()
                except Exception as e:
